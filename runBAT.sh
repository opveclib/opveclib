...
# Run Build Acceptance Tests for opveclib on python 2 and 3. Sstop after first failure
...
nose2-2.7 -F opveclib.test -A '!regression'
if [ ! $? -eq 0 ]; then
  echo "unit tests failed on Python 2"
  exit 1
fi
# now run in python 3
nose2-3.4 -F opveclib.test -A '!regression'
if [ ! $? -eq 0 ]; then
