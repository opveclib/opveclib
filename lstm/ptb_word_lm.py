...
class PTBModel(object):
  ...
  def __init__(self, is_training, config):
    ...
    # Changed from tensorflow - use our local copy of the lstm cells that
    # can call OVL instead
    # lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=True)
    from rnn_cell import BasicLSTMCell, DropoutWrapper, MultiRNNCell
    lstm_cell = BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=True)
...
def run_epoch(session, model, peak_wps, data, eval_op, verbose=False):
  """Runs the model on the given data."""
  epoch_size = ((len(data) // model.batch_size) - 1) // model.num_steps
  start_time = time.time()
  costs = 0.0
  iters = 0
  cur_peak_wps = peak_wps
  ...
  for step, (x, y) in enumerate(reader.ptb_iterator(data, model.batch_size,
    ...
    if verbose and step % (epoch_size // 10) == 10:
      ...
      if wps > cur_peak_wps:
        cur_peak_wps = wps
  return np.exp(costs / iters), cur_peak_wps
